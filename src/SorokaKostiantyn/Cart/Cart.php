<?php namespace SorokaKostiantyn\Cart;

use SorokaKostiantyn\Cart\Exceptions\InvalidConditionException;
use SorokaKostiantyn\Cart\Exceptions\InvalidItemException;
use SorokaKostiantyn\Cart\Helpers\Helpers;
use SorokaKostiantyn\Cart\Validators\CartItemValidator;

/**
 * Class Cart
 * @package SorokaKostiantyn\Cart
 */
class Cart
{
    /**
    ...
    public function __construct($session, $events, $instanceName, $session_key, $config)
    {
        ...
        $this->sessionKeyCartItems = $this->sessionKey . '_itemsInCart';
        $this->sessionKeyCartConditions = $this->sessionKey . '_cartConditions';
        $this->config = $config;
    ...
    public function add($id, $name = null, $price = null, $quantity = null, $attributes = [], $conditions = [])
    {
        ...
        $cart->has($id) ? $this->update($id, $item) : $this->addRow($id, $item);
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            ...
            if ($itemCondition instanceof $conditionInstance) {
                ...
                // to avoid hitting this error "Indirect modification of overloaded element of SorokaKostiantyn\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition)
            throw new InvalidConditionException(
                'Argument 1 must be an instance of \'SorokaKostiantyn\Cart\CartCondition\''
            );
    ...
    public function getConditionsByType($type)
    {
        return $this->getConditions()->filter(
            function (CartCondition $condition) use ($type) {
    ...
    {
        $this->getConditionsByType($type)->each(
            function ($condition) {
     ...
     * If you wish to remove a specific condition to a product, you may use the method: removeItemCondition($itemId,
     * $conditionName)
     *
     * @return void
     */
    ...
    public function getSubTotal($formatted = true)
    {
        ...
        $conditions->each(
            function (CartCondition $cond) use ($sum, &$newTotal, &$process) {
    ...
    public function getTotal()
    {
        ...
        $conditions->each(
                function (CartCondition $cond) use ($subTotal, &$newTotal, &$process) {
